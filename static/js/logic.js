...
function createMap(earthquakes) {
  ...
  var satelliteMap = L.tileLayer("https://api.mapbox.com/styles/v1/mapbox/satellite-v9/tiles/256/{z}/{x}/{y}?" +
    "access_token=pk.eyJ1IjoiYmNiYXVjb20iLCJhIjoiY2p6a2ZoOTd5MGtlMDNtbzkzeGNuYmR4OSJ9.cN9zBOUBLOtKqf_8MzwW6Q");
  ...
  legend.onAdd = function (myMap) {
    var div = L.DomUtil.create('div', 'info legend'),
              grades = [0, 1, 2, 3, 4, 5],
              labels = [];
    var colors = ['#0000ff','#cc0000' , '#ff6600' , '#ffff00','#996633', '#00cc00'];
  ...
  for (var i = 0; i < grades.length; i++) {
    ...
    '<i style="background:' + getColor(grades[i] + 1) + '"> &nbsp;&nbsp;&nbsp;&nbsp; </i> ' +
        "<span style='color: white'>" + (grades[i] + (grades[i + 1] ? '&ndash;' + grades[i + 1] + '<br>' : '+')) + "</span>";
    var htmllegend =
        '<i style="background:' + getColor(grades[i] + 1) + '">&nbsp;&nbsp;&nbsp;&nbsp;</i> ' +
            "<span style='color: white'>" + (grades[i] + (grades[i + 1] ? '&ndash;' + grades[i + 1] + '<br>' : '+')) + "</span>";
